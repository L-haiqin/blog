...
var sortList = function (head) {
  ...
  slow.next = null; // 把“当前 slow 指向的那个节点”的 next 指针改成 null。改变了head节点本身
  // 4 个节点：slow 在 2，左=[4,2]，右=[1,3]
  // 3 个节点：slow 在 2，左=[4,2]，右=[1]（左 2 右 1，归并没问题）
...
function merge(l1, l2) {
    ...
    if (l1) prev.next = l1;
    if (l2) prev.next = l2; // p.next = l1 || l2;
