...
/**
 * 二叉树最大宽度
 * 思路：给节点编号，根=1，左子=2*index，右子=2*index+1
 * 每层宽度 = 该层最大编号 - 最小编号 + 1，取所有层的最大值
 */
function widthOfBinaryTree(root) {
  if (!root) return 0;

  let maxWidth = 0;
  const queue = [[root, 1]]; // [节点, 编号]，根节点编号为 1
  ...
  while (queue.length) {
    ...
    while(levelSize) {
      ...
      minIndex = Math.min(minIndex, index); // 每一层的下标最大值，最小值
      maxIndex = Math.max(maxIndex, index);

      if (node.left) queue.push([node.left, index * 2]); // 为下一层做准备
      if (node.right) queue.push([node.right, index * 2 + 1]);

      levelSize--;
    }

    // 每一层的下标值计算完成后，再计算这一层的宽度
...
```
    ...
    (2)  3   2 (3)
       ...
       (4) 5   3   9 (7)
              (5)
**为什么是根=1，左子=2×index，右子=2×index+1？**

这是用数组存 完全二叉树 时的标准下标规则（1-based）：
...
这样一层层写出来，同一层的编号正好是连续的一段，且从左到右递增：
