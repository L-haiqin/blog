...
具体项目中，根据版本字符串生产版本号，可以用于一些rn version配置:

```js
const getVersionNumber = (versionText) => {
  if (!versionText) return 0;
  const versionNumber = versionText
    .split('.')
    .reverse()
    .reduce((accu, curVal, idx) => {
      const version = parseInt(curVal, 10);
      // if (version >= 100) {
      //   throw new Error("version number shouldn't exceed 100");
      // }
      return accu + version * 10 ** (idx * 3); // 每一位限制最长3位数
    }, 0);
  // NOTE: this is to prevent from floating point imprecision
  return Math.round(versionNumber);
};

console.log(getVersionNumber('1.22.3')) // 1022003

// 计算示例（'1.22.3'）：
// idx=0 ('3'): 3 * 10^0 = 3
// idx=1 ('22'): 22 * 10^3 = 22000
// idx=2 ('1'): 1 * 10^6 = 1000000
// 总和: 1000000 + 22000 + 3 = 1022003
```
...
使用栈的方式处理运算法优先级；遍历字符串，分别处理数字和运算符
 ...
 * 支持基本的四则运算（+、-、*、/），遵循运算符优先级
 * 
 * @param expression - 要计算的表达式字符串，例如 '3+2*3'
 * @returns 计算结果
 * 
 * @example
 * calculator('3+2*3') // 返回 9
 * calculator('10-5/2') // 返回 7.5
 */
...
function format(tpl, tplData) {
    ...
    return tpl.replace(/{{(.*?)}}/g,($0, value)=>{
        ...
        value.split('.').forEach(i=>{
            res = res[i]; 
            // 让变量 res 指向别的东西
            // res.b = 99 或 res["b"] = 99， 会改变tplData的值
        })
        return res;
    })
}
