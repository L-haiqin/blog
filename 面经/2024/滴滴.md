...
> React 内部用 Hook 的调用顺序 来把某次调用对应到某个 state/effect：
>
> - 第一次渲染：第 1 次调用 useState → 对应 state A，第 2 次 → state B，第 3 次 → state C…
>
> - 第二次渲染：仍然要求顺序、次数完全一致，才能把「这次的第 1 个 useState」还是当成 state A，以此类推。
>
> 也就是说：React 不靠“名字”或“变量”来认 Hook，只靠“第几个被调用的 Hook”。
>
> 如果放在条件语句里，他的调用顺序就不能得到保证。
